...
label1 = Label(text="Water Bill and Receipts", font=('impact', 32, 'bold'), fg="black", bg="white")
...
def check_function():
    ...
    if Unit_entry.get() == "" or tenant_entry.get() == "" or Number_entry.get() == "":
        ...
        with conn:
            cursor = conn.cursor()
        cursor.execute("CREATE TABLE IF NOT EXISTS WaterBill (id INTEGER PRIMARY KEY, Tenant_Name TEXT, House_Number TEXT, Units INTEGER)")
        cursor.execute('INSERT INTO WaterBill (Tenant_Name,House_Number,Units) VALUES(?,?,?)',(name, House_Number, Units))
        conn.commit()
def back():
    app.destroy()
    import Electricity
